...
TEST(OSQPEigenTests, MPCTestUpdatematrices)
{
    // open the ofstream
    ...
    for (int i = 0; i < numberOfSteps; i++)
    {
        ...
        EXPECT_TRUE(updateHessianMatrix(solver, Q, R, mpcWindow, i));
        EXPECT_TRUE(updateLinearConstraintsMatrix(solver, mpcWindow, i));

        castMPCToQPGradient(Q, yRef, mpcWindow, i, gradient);
        EXPECT_TRUE(solver.updateGradient(gradient));
